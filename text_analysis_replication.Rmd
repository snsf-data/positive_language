...
# .. .. using probit regression models
...
FI_column <- c(
  rep(paste0("Projects: ", format(nrow(pf_reg_ls$nbcount$model), big.mark = "'"),
             " observations"), 6),
  rep(paste0("Careers: ", format(nrow(career_reg_ls$nbcount$model),
                                 big.mark = "'"), " observations"), 6), 
  rep(paste0("Spark: ", format(nrow(sparkabs_reg_ls$nbcount$model),
                               big.mark = "'"), " observations"), 6))
...
tab_binary %>% 
  ...
  add_header_row(values = c(" ", "Logistic model for presence", "Negative binomial model for the count"), 
                 colwidths = c(1, 1, 1), top = TRUE) %>% 
  bold(bold = TRUE, part = c("header")) %>%
  set_caption(caption = "Summary of the logistic model using odds ratios (OR) and summary of the negative binomial model using rate ratios (RR), with 95% confidence intervals (CI). The reference for the gender variable is female and for the institution (inst.) type factor variable is 'Cantonal University'.") %>%
  width(width = 3) %>% 
  hline(i = c(7, 14)) %>% 
  bold(i = c(1, 8, 15))
```
...
spark_pred <- get_predict_words(sparkabs_reg_ls$nbcount,
                                freq_data = sparkabs_freq_ls)

pf_pred <- get_predict_words(pf_reg_ls$nbcount,
                             freq_data = pf_freq_ls)

career_pred <- get_predict_words(career_reg_ls$nbcount,
...
Figure \@ref(fig:fig-interaction-gender) illustrates the effects of gender on the language used in the negative binomial regression models. The plots show the predicted number of positive words with 95% confidence error bars per gender, for an applicant of average age with a proposal of average text length from a cantonal university, for each funding scheme. 
...
```{r fig-interaction-gender, fig.width = 6,fig.height = 3.5, fig.cap = "Illustration of the effects of gender on the language used in the negative binomial  regression models. The plots show the predicted number of positive words with 95% confidence error bars per gender, for an applicant of average age with a proposal of average text length, for each funding scheme. In all three funding schemes, male applicants used more positive words, but there was no significant difference as their confidence intervals overlap.", cache = FALSE, results = "asis", warning = FALSE, message = FALSE}
